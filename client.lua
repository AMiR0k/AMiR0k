...
local function requestEnterFarm(uuid)
    if farmTransitioning then return end
    if isBlocked() then return notify('در حالت مرگ نمی‌توانید وارد شوید.', 'error') end

    farmTransitioning = true
    DoScreenFadeOut(650)
    while not IsScreenFadedOut() do Wait(0) end

    local callbackOk, response = pcall(lib.callback.await, 'amirok_farming:enterFarm', false, uuid)
    if not callbackOk or not response or not response.ok then
        if IsScreenFadedOut() then DoScreenFadeIn(650) end
        farmTransitioning = false
        return notify((response and response.message) or 'ورود به مزرعه پاسخ نداد، لطفاً دوباره تلاش کنید.', 'error')
    end

    currentFarm = response.farm
    insideFarm = true
    SetEntityCoords(cache.ped, Config.Farm.spawn.x, Config.Farm.spawn.y, Config.Farm.spawn.z, false, false, false, true)
    Wait(250)
    DoScreenFadeIn(650)
    rebuildPlants(response.plants)
    farmTransitioning = false
end

local function requestExitFarm()
    if farmTransitioning then return end
    if isBlocked() then return notify('در حالت مرگ نمی‌توانید خارج شوید.', 'error') end

    farmTransitioning = true
    DoScreenFadeOut(650)
    while not IsScreenFadedOut() do Wait(0) end

    local callbackOk, response = pcall(lib.callback.await, 'amirok_farming:exitFarm', false)
    if not callbackOk or not response or not response.ok then
        if IsScreenFadedOut() then DoScreenFadeIn(650) end
        farmTransitioning = false
        return notify((response and response.message) or 'خروج از مزرعه پاسخ نداد، لطفاً دوباره تلاش کنید.', 'error')
    end
    ...
    currentPlants = {}
    deleteProps()
    SetEntityCoords(cache.ped, Config.Farm.returnTo.x, Config.Farm.returnTo.y, Config.Farm.returnTo.z, false, false, false, true)
    Wait(250)
    DoScreenFadeIn(650)
    farmTransitioning = false
end
