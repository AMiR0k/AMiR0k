...
local function isPlayerInRegisteredTug(source, netId)
    local state = getState(source)
    if not state.tugNetId or not state.tugPlate or type(netId) ~= 'number' then
        return false
    end

    -- First trust the server-registered Network ID that was returned by the server spawn callback.
    -- This avoids false rejections on boats where server-side GetVehiclePedIsIn/plate reads can be unreliable.
    if netId ~= state.tugNetId then
        return false
    end

    local registeredEntity = NetworkGetEntityFromNetworkId(state.tugNetId)
    if registeredEntity ~= 0 and DoesEntityExist(registeredEntity) and GetEntityModel(registeredEntity) ~= Config.TugModel then
        return false
    end
    ...
    if vehicle == 0 then
        -- Some artifacts do not reliably expose boat occupants server-side; the net id check above is the authority.
        return true
    end
    ...
    return registeredPlate ~= '' and currentPlate == registeredPlate
end
...
lib.callback.register('oilrunner:server:beginLoadOil', function(source, netId)
    ...
    if distanceTo(source, Config.LoadOil) > Config.RouteValidationDistance then
        return response(false, Config.Text.tooFar)
    end
...
lib.callback.register('oilrunner:server:beginDeliverOil', function(source, netId)
    ...
    if distanceTo(source, Config.DeliverOil) > Config.RouteValidationDistance then
        return response(false, Config.Text.tooFar)
    end
...
lib.callback.register('oilrunner:server:returnTug', function(source, netId)
    ...
    if distanceTo(source, Config.ReturnTug) > Config.RouteValidationDistance then
        return response(false, Config.Text.tooFar)
    end
